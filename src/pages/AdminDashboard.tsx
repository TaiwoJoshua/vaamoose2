...
import { Bus, CheckCircle2, X, Shield, ArrowLeft, Phone, Mail } from 'lucide-react';
  ...
  const handleApprove = async (partnerId: string) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/admin/partners/${partnerId}/approve`,
        {
          method: 'PUT',
          headers: { 'x-admin-key': ADMIN_KEY },
        }
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardContent className="p-8">
            ...
            <button
              onClick={onBack}
              className="w-full mt-3 text-slate-500 hover:text-slate-700 text-sm flex items-center justify-center gap-1"
            >
